...
export const AskMonishAIChat: React.FC<AskMonishAIChatProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/80 backdrop-blur-md">
      <div className="relative w-full max-w-2xl bg-[#0c0f16] border border-white/[0.12] rounded-2xl shadow-2xl overflow-hidden flex flex-col h-[650px] max-h-[92vh]">
        {/* Chat Header */}
        ...
        <div className="flex-1 p-4 sm:p-6 overflow-y-auto space-y-4">
          {messages.map((msg) => (
            ...
            >
              {msg.role === 'assistant' && (
                <div className="w-7 h-7 rounded-lg bg-blue-500/10 border border-blue-500/30 flex items-center justify-center text-blue-400 shrink-0 mt-1">
              ...
              <div
                className={`relative group max-w-[85%] rounded-2xl p-4 text-xs sm:text-sm leading-relaxed ${
                  msg.role === 'user'
                    ...
                    : 'bg-slate-900/80 text-slate-200 border border-white/[0.08] rounded-tl-xs shadow-sm'
                }`}
              >
                <div className="whitespace-pre-wrap">{msg.content}</div>

                <div className="flex items-center justify-between mt-2 pt-1 border-t border-white/[0.06] text-[10px] text-slate-400">
        ...
        <div className="px-4 py-2.5 bg-slate-950/60 border-t border-white/[0.06] overflow-x-auto flex gap-2">
          {SUGGESTED_CHAT_PROMPTS.map((prompt, i) => (
            <button
              key={i}
              onClick={() => handleSendMessage(prompt)}
              className="px-2.5 py-1 rounded-lg text-[11px] font-mono text-slate-300 hover:text-white bg-slate-900 hover:bg-slate-800 border border-white/[0.06] whitespace-nowrap cursor-pointer transition-colors"
            >
              {prompt}
            </button>
          ))}
        ...
        {/* Input Bar */}
        <div className="p-3 sm:p-4 bg-slate-900/60 border-t border-white/[0.08]">
          <form
            onSubmit={(e) => {
              e.preventDefault();
              handleSendMessage();
          ...
          >
            <button
              ...
              className={`p-2.5 rounded-xl border transition-colors cursor-pointer ${
                isListening
                  ...
                  : 'bg-slate-800 text-slate-400 hover:text-white border-white/[0.06]'
              }`}
            >
            ...
            <input
              ...
              className="flex-1 px-4 py-2.5 rounded-xl bg-slate-950 border border-white/[0.1] text-xs sm:text-sm text-white placeholder-slate-500 focus:outline-none focus:border-blue-500 font-sans"
            />
